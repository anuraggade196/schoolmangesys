# schoolmangesys
I developed this portal using python PHP used NoSQL for data mangement .
import datetime
from tkinter import *
...
connector = sqlite3.connect('SchoolManagement1.db')
cursor = connector.cursor()

connector.execute(
"CREATE TABLE IF NOT EXISTS SCHOOL_MANAGEMENT (STUDENT_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, NAME TEXT, EMAIL TEXT, PHONE_NO TEXT, GENDER TEXT, DOB TEXT, STREAM TEXT, ADHAR_NO TEXT)"
)

# Creating the functions
def reset_fields():
    global name_strvar, email_strvar, contact_strvar, gender_strvar, dob, stream_strvar, adhar_strvar

    for i in ['name_strvar', 'email_strvar', 'contact_strvar', 'gender_strvar', 'stream_strvar', 'adhar_strvar']:
        exec(f"{i}.set('')")
...
def add_record():
    ...
    adhar = adhar_strvar.get()

    if not name or not email or not contact or not gender or not DOB or not stream or not adhar:
        mb.showerror('Error!', "Please fill all the missing fields!!")
    else:
        try:
            connector.execute(
            'INSERT INTO SCHOOL_MANAGEMENT (NAME, EMAIL, PHONE_NO, GENDER, DOB, STREAM, ADHAR_NO) VALUES (?,?,?,?,?,?,?)', (name, email, contact, gender, DOB, stream, adhar)
            )
            connector.commit()
...
lf_bg = 'Yellow' # bg color for the left_frame
cf_bg = 'Grey' # bg color for the center_frame
...
Label(left_frame, text="Adhar Number.", font=labelfont, bg=lf_bg).place(relx=0.2, rely=0.80)
...
Entry(left_frame, width=19, textvariable=adhar_strvar, font=entryfont).place(x=20, rely=0.85)
...
Button(left_frame, text='Submit and Add Record', font=labelfont, command=add_record, width=18).place(relx=0.025, rely=0.90)
...
tree = ttk.Treeview(right_frame, height=100, selectmode=BROWSE,
                    columns=('Student ID', "Name", "Email Address", "Contact Number", "Gender", "Date of Birth", "Stream", "Adhar number"))
...
tree.heading('Adhar number', text='Adhar number', anchor=CENTER)
